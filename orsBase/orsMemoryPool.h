...
//*************************************************************************
//
// $Id: orsMemoryPool.h 2009/9/2 15:45 JWS $
//
// Project: OpenRS
//
// Purpose: 
...
//*************************************************************************
//
// $Log: orsMemoryPool.h,v $
//
// Revision 1.0 date: 2009/9/2 by JIANG Wanshou
// new
//

#if !defined(_ORS_MEMORYPOOL_H_INCLUDED_)
#define _ORS_MEMORYPOOL_H_INCLUDED_
...
template<class TYPE>
orsMemoryPool<TYPE>::orsMemoryPool()
{
...
/*****************************************************************************/
template<class TYPE>
void orsMemoryPool<TYPE>::PoolInit(int itemcount)
{
...
template<class TYPE>
void orsMemoryPool<TYPE>::PoolDeinit()
{
...
template<class TYPE>
bool orsMemoryPool<TYPE>::IncreaseABlock()
{
	dataBLOCK *blocks;

	if( m_blockSum == m_maxBlockSum ) {
		m_maxBlockSum += 1024;
		try{  //zbr 增加异常处理，分配不成功return false
...
template<class TYPE>
void orsMemoryPool<TYPE>::PoolRestart()
{
	if( m_blockSum ) {
		m_numberOfItems = 0;
...
template<class TYPE>
TYPE *orsMemoryPool<TYPE>::PoolAlloc()
{
	TYPE *newItem;
...
template<class TYPE>
void orsMemoryPool<TYPE>::PoolDealloc(TYPE *dyingitem)
{
...
template<class TYPE>
void orsMemoryPool<TYPE>::TraversalInit()
{
...
template<class TYPE>
TYPE *orsMemoryPool<TYPE>::Traverse()
{
	TYPE *newitem;
...
template<class TYPE>
TYPE *orsMemoryPool<TYPE>::GetItem( int number )
{
...
template<class TYPE>
TYPE *orsMemoryPool<TYPE>::GotoItem( int number )
{
	int blockNum, itemNum;
